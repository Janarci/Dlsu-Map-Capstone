...
public class BefriendingGestures : MonoBehaviour
{
    ...
    [Header("Befriending Sprites")]
    [SerializeField] Sprite fbEmpty;
    [SerializeField] Sprite fbFilled;
    [SerializeField] Sprite cfIdle;
    [SerializeField] Sprite cfPouring;
        ...
        switch(currentMode)
        {
            ...
            case BefriendingMode.Petting:
                ...
                if (currentProgress >= maxProgress)
                {
                    ...
                    AudioManager.Instance.Stop("Gesture Pet", "sfx");
            ...
            case BefriendingMode.Feeding:
                ...
                if (currentProgress >= maxProgress)
                {
                    ...
                    AudioManager.Instance.Stop("On Food Bowl Fill", "sfx");
                    AudioManager.Instance.Play("Gesture Eat", "sfx", false);
            ...
            case BefriendingMode.Cleaning:
                ...
                if (currentProgress >= maxProgress)
                {
                    ...
                    showerHead.GetComponent<HandCloth>().ResetIntersectedDirt();
                    ...
                    AudioManager.Instance.Stop("Gesture Clean", "sfx");
            ...
            case BefriendingMode.Petting:
                {
                    if (pettingHand.GetComponent<BefriendingObjects>().isSelected)
                    {
                        ...
                        if (b1.Intersects(b3))
                        {
                            ...
                            AudioManager.Instance.Play("Gesture Pet", "sfx", true);
                        }

                        else
                        {
                            AudioManager.Instance.Pause("Gesture Pet", "sfx");
                        }
            ...
            case BefriendingMode.Feeding:
                {
                    ...
                    if(foodPackComp.isSelected)
                    {
                        ...
                        {
                            Debug.Log("feeding");
                            if(foodPack.GetComponent<Image>().sprite != cfPouring)
                            {
                                foodPack.GetComponent<Image>().sprite = cfPouring;
                            }

                            if(foodBowl.GetComponent<Image>().sprite != fbFilled)
                            {
                                foodBowl.GetComponent<Image>().sprite = fbFilled;
                            }
                            feedingProgress += 30.0f * Time.deltaTime;

                            AudioManager.Instance.Play("On Food Bowl Fill", "sfx", true);

                        }

                        else
                        {
                            if (foodPack.GetComponent<Image>().sprite != cfIdle)
                            {
                                foodPack.GetComponent<Image>().sprite = cfIdle;
                            }

                            if (foodBowl.GetComponent<Image>().sprite != fbEmpty)
                            {
                                foodBowl.GetComponent<Image>().sprite = fbEmpty;
                            }
                            feedingProgress = 0.0f;
                            AudioManager.Instance.Pause("On Food Bowl Fill", "sfx");
            ...
            case BefriendingMode.Playing:
                {
                    ...
                    {
                        Bounds b1 = new Bounds(playingYarn.transform.position, playingYarn.GetComponent<RectTransform>().rect.size);
                        Bounds b2 = cat.gameObject.GetComponent<Collider>().bounds;
                        Bounds b3 = new Bounds(Vector3.zero, Vector3.zero);
                        b3.center = Camera.main.WorldToScreenPoint(b2.center);
                        b3.min = Camera.main.WorldToScreenPoint(b2.min);
                        ...
                        if (b1.Intersects(b3))
                        {
                            ...
                            //else
                            //{
                            //    cat.GetComponent<CatAnimsTest>().cat_play();
                            //}
            ...
            case BefriendingMode.Cleaning:
                {
                    if (showerHead.GetComponent<BefriendingObjects>().isSelected)
                    {
                        ...
                        float _cleaningProgress = cleaningProgress;
                        int cleanedUpDirt = 0;
                        for (int i = 0; i < dirt.transform.childCount; i++)
                        {
                            ...
                            else if (showerHead.GetComponent<HandCloth>().isCleaningUpDirt())
                            {
                                if(showerHead.GetComponent<HandCloth>().GetCoveredDirt().Contains(dirt.transform.GetChild(i).gameObject))
                                {
                        ...
                        if(_cleaningProgress < cleaningProgress)
                        {
                            AudioManager.Instance.Play("Gesture Clean", "sfx");
                        }

                        else
                        {
                            AudioManager.Instance.Pause("Gesture Clean", "sfx");
                        }
