...
public class HQUI : MonoBehaviour
{
    ...
    private void OnCatSelect(Cat selectedCat)
    {
        if(focusedCat == null || focusedCat != selectedCat)
        {
            ...
            hqCam.LookAtCat(focusedCat.gameObject);
            //focusedCat.StopRoaming();
            focusedCat.GetComponent<catAgent>().StopRoam();
            Debug.Log("meow mewo");
    ...
    public void UnselectCat()
    {
        if(focusedCat)
        {
            ...
            focusedCat.GetComponent<catAgent>().ResumeRoam();
            focusedCat = null;
            hqCam.ResetCamera();
            UpdateCatAvailableEvolutions();
            UpdateCatMaterialsList();
    ...
    private void CameraFocusOnCat(GameObject selectedCat)
    {
        ...
        Vector3 cameraNewPosition = Vector3.Lerp(hqCam.transform.position, catFrontPosition, zoomTvalue);
    ...
    public void DisplayCatToolTip()
    {
        if(catTooltipUI)
        {
            if (!(CatsManager.instance.collected_cat_types.Contains(focusedCat.GetCatType())))
            {
                CatsManager.instance.UnlockCatType(focusedCat.GetCatType());
    ...
    public void OnCatEvolve(Cat oldCat, Cat newCat, CatType.Type evolvedType)
    {
        ...
        //if(!CatsManager.instance.collected_cat_types.Contains(evolvedType))
        //{
    ...
    public void OnQuestBtnPress()
    {
        questsUI.SetActive(!questsUI.gameObject.activeInHierarchy);
    }
