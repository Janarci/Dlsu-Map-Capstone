...
public class PopupGenerator : MonoBehaviour
{
    ...
    [SerializeField] private GameObject infoUICloseableGeneral;
    [SerializeField] private GameObject infoUICloseableTutorial;
    ...
    private void Update()
    {
        if(popupQueue.Count != 0 && infoUIObj == null)
        {
            ...
            AudioManager.Instance?.Play("Popup Open", "sfx", false);
        }

    }

    public GameObject GenerateTutorialPopup(string popupMessage)
    {
    ...
    public void GenerateCloseablePopup(string popupMessage)
    {
        ...
        if (mainCanvas)
        {
            GameObject newInfoUIObj = Instantiate(infoUICloseableGeneral, mainCanvas.transform);
            newInfoUIObj.transform.GetChild(0).GetComponent<Text>().text = popupMessage;
            newInfoUIObj.transform.GetChild(1).GetComponent<Button>().onClick.AddListener(delegate { Destroy(newInfoUIObj); infoUIObj = null; AudioManager.Instance?.Play("Popup Close", "sfx", false); });
            dontOverwrite = false;
